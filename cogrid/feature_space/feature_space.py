...
from envs.gridworld.feature_space import features
...
class FeatureSpace:
    def __init__(self, feature_names: list[str], env, agent_id: str, **kwargs):
        ...
        self.observation_space = Dict({feature.name: feature.space for feature in self.feature_generators})
        self.agent_id = agent_id

    def generate_features(self):
        generated_features = {
            feature.name: feature.generate(self.env, self.agent_id) for feature in self.feature_generators
        }
        return generated_features

    def _setup_feature_generators(self, feature_names):
        return [self._fetch_feature_generator(feature_name) for feature_name in feature_names]

    def _fetch_feature_generator(self, feat_name):
        ...
        if feat_name == "other_agent_actions":
            return features.OtherAgentActions(len(self.env.agent_ids), self.env.action_space.n)
        ...
        if feat_name == "other_agent_visibility":
            return features.OtherAgentVisibility(len(self.env.agent_ids), self.env.agent_view_size)
        if feat_name == "role":
