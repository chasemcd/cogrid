...
def test_obs_shape_210(layout_name):
    """Each agent should get a (892,) observation."""
    env = _make_env(layout_name)
    obs, _ = env.reset(seed=42)
    for aid in env.possible_agents:
        assert obs[aid].shape == (892,), f"Agent {aid} obs shape {obs[aid].shape} != (892,)"
...
def test_step_preserves_shape():
    """Observation shape should remain (892,) after stepping."""
