...
from cogrid.core.objects import GridObj
...
# Raw direction vectors: Right=0, Down=1, Left=2, Up=3.
# Each row is [delta_row, delta_col].
_DIR_VECS = [[0, 1], [1, 0], [0, -1], [-1, 0]]


def get_dir_vec_table():
    """Return the (4, 2) direction vector lookup table.
    ...
    Always uses the current ``xp`` backend so the result is safe inside
    JAX JIT.
    """
    return xp.array(_DIR_VECS, dtype=xp.int32)
