"""Array-based reward functions operating on state dictionaries instead of Grid objects.

Each reward function has the signature:
    reward_fn(prev_state, state, actions, type_ids, n_agents, ...) -> ndarray of shape (n_agents,)

These functions exist alongside the existing Reward classes (cogrid.core.reward /
cogrid.envs.overcooked.rewards). Integration into the step loop is deferred to Plan 07.
