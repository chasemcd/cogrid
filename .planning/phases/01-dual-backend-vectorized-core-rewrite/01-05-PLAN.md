...
Run: `python -m pytest cogrid/test_gridworld_env.py cogrid/test_overcooked_env.py -x -q` -- all pass (regression check).
  </verify>
  <done>
Feature parity test confirms array-based features produce numerically identical outputs to existing Feature.generate() system for agent_pos, agent_dir, full_map_encoding, can_move_direction, and inventory across 20 random steps. All existing tests pass.
  </done>
</task>
