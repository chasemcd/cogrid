...
must_haves:
  truths:
    ...
    - "Backend parameter is accepted and calls set_backend() with lazy-once semantics"
  ...
  key_links:
    - from: "cogrid/cogrid_env.py"
      to: "cogrid/backend"
      via: "set_backend() call in __init__ with lazy-once semantics"
      pattern: "set_backend"
    - from: "cogrid/cogrid_env.py"
...
2. Call `set_backend(backend)` which implements lazy-once initialization per user decision ("Backend dispatch: global resolution, selection via first env"):
   ```python
   from cogrid.backend import set_backend
   set_backend(backend)
   ```
   Per the locked decision, the backend is selected by the FIRST environment created, with global resolution thereafter. `set_backend()` in `_dispatch.py` (implemented in Plan 01-01) enforces this via the `_backend_set` guard: the first call sets the backend globally, and any subsequent call with a DIFFERENT backend raises `RuntimeError`. Calling with the SAME backend is a no-op. This means:
   - First `CoGridEnv(..., backend='numpy')` sets global backend to numpy
   - Second `CoGridEnv(..., backend='numpy')` is a no-op (same backend)
   - `CoGridEnv(..., backend='jax')` after numpy was set raises RuntimeError
   - The default `backend='numpy'` means existing code without the parameter works unchanged
...
Run: `python -m pytest cogrid/ -x -q --timeout=60` -- all tests pass.
  ...
  <done>
CoGridEnv accepts `backend='numpy'` parameter and calls set_backend() with lazy-once semantics (first env sets backend globally, subsequent envs verify match per locked decision "selection via first env"). Array state created on reset(). Vectorized movement used as primary path with sync back to Agent objects. Existing interactions, features, rewards continue working on synced objects. All existing tests pass without modification.
  </done>
</task>
...
3. Backend parameter accepted and set_backend() called with lazy-once semantics (per locked decision: "selection via first env")
...
- Backend dispatch module is called on environment creation with lazy-once semantics (first env sets, subsequent verify match)
