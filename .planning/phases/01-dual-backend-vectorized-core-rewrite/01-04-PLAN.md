...
Run: `python -m pytest cogrid/test_gridworld_env.py cogrid/test_overcooked_env.py -x -q` -- all pass (regression check).
  ...
  <done>
Parity test confirms array-based interactions match existing object-based interactions for random and deterministic test cases. Pickup, drop, place-on, and pot cooking all produce identical results. All existing tests pass.
  </done>
</task>
