...
must_haves:
  truths:
    - "jax.jit(get_obs_jax)(state) executes without ConcretizationTypeError"
    - "Per-agent observation generation uses jax.vmap instead of Python loop"
    - "jax.jit(compute_rewards_jax)(prev_state, state, actions) executes without error"
  ...
  artifacts:
    ...
    - path: "cogrid/envs/overcooked/array_rewards.py"
      provides: "JAX-path reward functions"
      contains: "delivery_reward_jax"
  key_links:
...
```
  ...
  <done>Three JAX-path reward functions exist. All use vectorized ops across agents instead of Python loops. Pot position lookup uses array matching. get_all_agent_obs_jax uses jax.vmap. All JIT-compile without error. Existing numpy functions unchanged.</done>
</task>
...
2. `get_all_agent_obs_jax` uses vmap and produces correct `(n_agents, obs_dim)` shape
3. All 3 JAX reward functions JIT-compile
4. No `int()` casts on traced values in JAX path
5. No Python loops over agents in JAX path
6. `_find_pot_index` Python loop replaced with array matching in JAX path
7. All existing numpy-path functions completely unchanged
</verification>

<success_criteria>
- `jax.jit(get_all_agent_obs_jax)(feature_fn, state_dict, n_agents)` executes without error
- `jax.jit(delivery_reward_jax)(...)`, `jax.jit(onion_in_pot_reward_jax)(...)`, `jax.jit(soup_in_dish_reward_jax)(...)` all execute without error
