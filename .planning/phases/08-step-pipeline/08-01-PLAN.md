...
Run: `python -c "
with open('cogrid/core/step_pipeline.py') as f:
    content = f.read()
assert 'move_agents(' in content, 'Missing move_agents call'
assert 'process_interactions(' in content, 'Missing process_interactions call'
assert 'get_all_agent_obs(' in content, 'Missing get_all_agent_obs call'
assert 'compute_rewards(' in content, 'Missing compute_rewards call'
assert 'tick_handler' in content, 'Missing tick_handler logic'
print('All sub-functions composed in step()')
"` -- verify all Phase 5-7 sub-functions are composed in the pipeline.
  </verify>
  <done>
