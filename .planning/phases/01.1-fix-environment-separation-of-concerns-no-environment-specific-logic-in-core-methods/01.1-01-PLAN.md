...
- `_overcooked_interaction_handler(...)` -- a function that handles the Overcooked-specific parts of `process_interactions_array()`. Specifically: (a) dynamic can_pickup_from condition for pots (dish_ready AND has plate), (b) pot pickup-from: soup type determination and plate consumption, (c) pot place-on via `_place_on_pot`, (d) delivery zone place-on via `_place_on_delivery_zone`. This function will be called by the refactored generic `process_interactions_array` in Plan 03.
