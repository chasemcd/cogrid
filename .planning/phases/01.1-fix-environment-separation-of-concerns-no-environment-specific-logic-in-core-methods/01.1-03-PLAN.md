...
**Part D: Update moved test call sites**

In `cogrid/envs/overcooked/test_interactions.py`:

1. Update the import of `tick_objects_array` to import from `cogrid.envs.overcooked.array_config` (the `_overcooked_tick_handler` function) instead of `cogrid.core.interactions`. The test should use the Overcooked-specific tick handler directly since it's testing Overcooked behavior.

2. Update ALL calls to `process_interactions_array()` in the test file. The old signature passes positional args: `(agent_pos, agent_dir, agent_inv, actions, otm, osm, pc, pt, pp, tables, type_ids, scope, dir_vec, PICKUP_DROP, toggle_idx, pickup_from_produces, legal_pot_ingredients)`. The new signature is: `(agent_pos, agent_dir, agent_inv, actions, object_type_map, object_state_map, lookup_tables, scope_config, dir_vec_table, action_pickup_drop_idx, action_toggle_idx, **extra_state)`. Each call site must:
   - Remove the old positional `pc, pt, pp, type_ids, scope, pickup_from_produces, legal_pot_ingredients` args
   - Add `scope_config` parameter (build once at top of test function via `get_scope_config("overcooked")`)
   - Pass pot arrays as keyword arguments in `**extra_state` (e.g., `pot_contents=pc, pot_timer=pt, pot_positions=pp`)
   - Update return value unpacking to include `extra_state` dict as the 4th return value: `(agent_inv, object_type_map, object_state_map, extra_state)` instead of the old 5-tuple `(agent_inv, otm, osm, pc, pt)`
   - Extract pot arrays from `extra_state` dict where needed (e.g., `pc = extra_state["pot_contents"]`)

There are approximately 10 call sites in the test function. Search for `process_interactions_array(` in the file and update each one.
