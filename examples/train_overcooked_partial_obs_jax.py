"""Train shared-parameter IPPO on CoGrid Overcooked with partial observability (JAX backend).

Same as train_overcooked_jax.py but uses the partial-observability environment
variant (observable_radius=3, local_view feature). Agents only perceive a 7x7
window around their position; state beyond the radius is masked.

Usage:
    python examples/train_overcooked_partial_obs_jax.py
...
ENVIRONMENT_NAME = "Overcooked-AsymmetricAdvantages-PartialObs-V0"
...
def visualize_policy(
    ...
    env_id,
    max_steps=1000,
    gif_path="examples/episode_partial_obs.gif",
    fps=30,
    seed=0,
):
...
def export_to_onnx(
    ...
    n_actions,
    activation="tanh",
    onnx_path="examples/policy_partial_obs.onnx",
):
...
if __name__ == "__main__":
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig("examples/overcooked_partial_obs_training.png", dpi=150)
        print("Saved learning curve to examples/overcooked_partial_obs_training.png")
    except ImportError:
        pass
